...
def calculate_cash_flow(
    engagement_id: int,
    input_data: schemas.CashFlowInput,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    """
    Calculates the Indirect Cash Flow Statement (DFC) based on Net Income (from system)
    and provided adjustments. Updates the context with these manual inputs.
    """
    # 1. Get/Update Context with manual inputs
    context = db.query(models.FinancialStatementContext).filter(
        models.FinancialStatementContext.engagement_id == engagement_id
    ).first()
    ...
    # Update context with DFC inputs
    # We store them so they persist for future GET requests (if we implement GET DFC)
    # Merging into context_data
    current_data = dict(context.context_data) if context.context_data else {}
    current_data["dfc_inputs"] = input_data.model_dump()
    context.context_data = current_data
    context.updated_at = datetime.utcnow()
    db.commit()

    # 2. Calculate Net Income (Automatic)
    # Ideally, we should reuse the logic from `generate`.
    # For simplicity/DRY, we'll re-run a simplified version or call an internal helper.
    # Re-implementing simplified logic here to avoid refactoring risk right now.
    ...
    balances = db.query(
        ...
        models.Transaction.engagement_id == engagement_id
    ).group_by(models.Transaction.account_code).all()
    ...
    # Revenue (3) is negative in DB, Expenses (4) positive.
    # Profit = -1 * Rev - Exp
    receita = -1 * sum(v for k, v in std_balances.items() if k.startswith("3"))
    despesa = sum(v for k, v in std_balances.items() if k.startswith("4"))
    net_income = receita - despesa

    # 3. Apply Adjustments (Indirect Method)
    # Operating Flow = Net Income + Adjustments
    operating_flow = net_income

    # Adjustments (Depreciation usually positive addition back to profit if profit had it deducted)
    # Input format: { "description": "Depreciation", "value": 50000 }
    # User is responsible for sign. Usually Depr is added back (+).
    total_op_adjustments = sum(item.value for item in input_data.operating_adjustments)
    operating_flow += total_op_adjustments

    # Investment
    investment_flow = sum(item.value for item in input_data.investment_activities)

    # Financing
    financing_flow = sum(item.value for item in input_data.financing_activities)

    # Net Increase/Decrease
    net_change = operating_flow + investment_flow + financing_flow

    return {
        "net_income": net_income,
        "operating_activities": {
            "adjustments": input_data.operating_adjustments,
            "total": operating_flow
        },
        "investment_activities": {
            "items": input_data.investment_activities,
            "total": investment_flow
        },
        "financing_activities": {
            "items": input_data.financing_activities,
            "total": financing_flow
        },
