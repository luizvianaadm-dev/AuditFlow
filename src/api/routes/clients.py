...
def upload_ledger(client_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    """
    ...
    try:
        # file.file is a SpoofedTemporaryFile (or similar) which is file-like.
        # For 'def' endpoints, FastAPI/Starlette doesn't auto-read file into bytes if we assume UploadFile.
        # However, file.file.read() is sync.
